...
Default landmark count is `min(500, ceiling(sqrt(n)))`. Inference (when
...
for (i in seq_along(sizes)) {
  ...
  t_e <- system.time({
    fit_e <- krls(tr$X, tr$y, derivative = FALSE, vcov = FALSE,
                  print.level = 0)
  })["elapsed"]
  t_n <- system.time({
    fit_n <- krls(tr$X, tr$y, approx = "nystrom", nystrom_m = m,
...
tr <- make_data(n)
te <- make_data(200)
m  <- ceiling(sqrt(n))   # ~45 landmarks

t_n <- system.time({
  fit_big <- krls(tr$X, tr$y, approx = "nystrom", nystrom_m = m,
                  derivative = FALSE, print.level = 0)
})["elapsed"]
